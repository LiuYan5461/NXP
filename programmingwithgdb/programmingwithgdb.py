...
class Application(Frame):
    ...
    def read_config_file(self,path):
        ...
        if os.path.exists(path):
            with open(path) as f:
                ...
                if lineNumber >= 3 :    
                    self.deviceNameList = contents[2].strip().split(',')
                    self.deviceName = self.deviceNameList[0]
                if lineNumber >= 4 :
                    self.rootDir.set(contents[3])
    ...
    def create_widgets(self):
        ...
        Entry_gdb.grid(row=0, padx=5, pady=4, columnspan=3, sticky='new')
        ...
        LabelFrame_jlinkDevice.grid(row=2, padx=4, pady=4, columnspan=3, sticky='new')
        ...
        Usage = ['Usage:\n\n',
                 '1. Set path for jlink gdb server and arm gdb.\n\n',
                 '2. Select jlink device, if not in the list, add a new one in the Entry box and then click add button.\n\n',
                 '3. Only binary with suffix \'elf\' and \'out\' will be searched. Modify src code to support a new one.\n',
                 ...
                 '4. Click search button to start to search. It may take a while to get the result and in this process,\n',
                 '   the application may not respond.\n\n',
                 '5. Double click one of item in the result list to download.\n\n',
                 '6. Don\'t forget to save changed configuration before exit.Happy Programming!'
                 ]
        label_usage = Label(self.containerFrame1, text=''.join(Usage), justify = LEFT)
        label_usage.grid(row=3, pady=5, padx=5,columnspan=3, sticky='nsw')
        ...
        self.armgccCheck.set(True)
        self.mdkCheck.set(True)
        self.iarCheck.set(True)
        self.kdsCheck.set(True)
        self.atlCheck.set(True)
        ...
        self.Listbox_searchResesult = Listbox(Labelframe_result, selectmode=BROWSE, height=20, width=60)
